...
const dashboardWs: FastifyPluginAsync = async (app) => {
  app.get('/ws/dashboard', { websocket: true }, async (socket, _request) => {
    ...
    const sessionHandler = (data: { sessionId: string; status: string }) => {
      ...
      if (session) {
        safeSend(socket, JSON.stringify({ type: 'session_update', session: withParsedGitMetadata(session) }));
      }
    };
    ...
    const notificationHandler = (data: {
      ...
      safeSend(
        socket,
        JSON.stringify({ type: 'notification', notification: data }),
      );
    };
